    ...
    tableint addPoint(const void *data_point, labeltype label, int level,
        ...
        if ((signed)currObj != -1) {
            if (curlevel < maxlevelcopy) {
                ...
                /* Percona: renamed from `level` — shadowed the addPoint
                parameter and broke -Werror=shadow builds. */
            ...
            /* Percona: renamed from `level` — see rename above. */
            for (int lvl = std::min(curlevel, maxlevelcopy); lvl >= 0; lvl--) {
                if (lvl > maxlevelcopy || lvl < 0)  // possible?
