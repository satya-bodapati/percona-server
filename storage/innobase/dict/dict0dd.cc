...
void dd_copy_table_columns(const Alter_inplace_info *ha_alter_info,
  ...
  for (const auto old_col : old_table.columns()) {
    if (old_col->is_se_hidden() && !is_system_column(old_col->name().c_str()) &&
        ...
        (strcmp(old_col->name().c_str(), VEC_IDX_ID_COL_NAME) != 0)) {
...
bool copy_dropped_columns(const dd::Table *old_dd_table,
  ...
  for (const auto column : old_dd_table->columns()) {
    ...
    if (!column->is_se_hidden() ||
        ...
        innobase_strcasecmp(col_name, VEC_IDX_ID_COL_NAME) == 0) {
      continue;
    }
...
static inline void fill_dict_columns(const Table *dd_table, const TABLE *m_form,
  ...
  if (add_vec_idx_id) {
    ...
    vec_add_idx_id_column(dict_table, heap);
  }
...
static inline dict_table_t *dd_fill_dict_table(const Table *dd_tab,
  ...
  /* Same shape for the vector vec_idx_id auxiliary column: detect it in
  ...
  bool add_vec_idx_id = false;
  {
    const dd::Column *vec_col =
        dd_find_column(&dd_tab->table(), VEC_IDX_ID_COL_NAME);
    if (vec_col != nullptr &&
  ...
  const unsigned n_cols =
      n_mysql_cols + (add_doc_id ? 1 : 0) + (add_vec_idx_id ? 1 : 0);
  ...
  if (!p->get(dd_index_key_strings[DD_TABLE_ID], &table_id)) {
    ...
    if (table->is_aux()) {
      ...
      if (table->is_fts_aux()) {
        ...
        /* Unlike FTS, the vec aux name matcher is prefix-only (see
        vec_aux_is_aux_table_name) so DICT_TF2_VEC_AUX can in theory
        be set on a name the strict parser rejects (e.g. a pre-feature
        user table "vec_foo" surviving an upgrade). Never consume an
        unparsed parent_id — skip the parent-MDL dance instead. */
        parent_known =
            vec_aux_parse_table_name(table->name.m_name, &parent_id, nullptr);
        ut_ad(parent_known); /* strict parse must succeed for real aux */
      }
...
bool dd_create_vec_aux_table(const dict_table_t *parent_table,
  ...
  descriptors (vec0aux.cc). One mapping per column kind the descriptors
  use: DATA_INT (1-, 4- or 8-byte), DATA_BLOB, DATA_BINARY.

  This replaces a hand-rolled per-column version that hardcoded one aux
  shape (the pre-H1 single-column PK on `id`). That is a trap worth
  naming: a descriptor whose shape the DD writer does not know reloads
  with the wrong columns and n_uniq, and stays latent until the FIRST
  RESTART — within one server run the dict cache still holds the
  correct in-memory objects, so nothing looks wrong. Deriving the DD
  entry from the in-memory table keeps the two definitions from
  drifting, for this TYPE's log and for any TYPE added later. */
  ...
  for (ulint i = 0; i < n_user_cols; ++i) {
    ...
    switch (dcol->mtype) {
      case DATA_INT:
        /* Widths the vec aux descriptors use: BIGINT (labels), INT (the
        H1 edge-list version) and TINYINT (levels). */
        switch (dcol->len) {
          case 8:
            col->set_type(dd::enum_column_types::LONGLONG);
            col->set_char_length(20);
  ...
  unique fields — the descriptor's leading n_pk columns, which is a
  COMPOSITE key for the H1 edge log (label, ver). */
  dd::Index *index = dd_table->add_index();
