...
bool Instant_ddl_impl<Table>::commit_instant_ddl() {
  ...
  switch (type) {
    ...
    case Instant_Type::INSTANT_VIRTUAL_ONLY:
      ...
      /* Mirror the FTS_DOC_ID re-add above for the hidden vec_idx_id
      ...
      DD on commit still describes vec_idx_id, otherwise next reload
      crashes in vec_add_idx_id_column / dd_open_table_one.
      ...
      (handler0alter.cc). Linkage from base to aux goes base.vec_idx_id
      → aux.id via each side's own PK: two PK lookups, no intermediate
      B-tree. See PS-11299 phase 1 design. */
      if (dd_find_column(&m_old_dd_tab->table(), VEC_IDX_ID_COL_NAME) &&
          !dd_find_column(&m_new_dd_tab->table(), VEC_IDX_ID_COL_NAME)) {
        dd_add_hidden_column(&m_new_dd_tab->table(), VEC_IDX_ID_COL_NAME,
