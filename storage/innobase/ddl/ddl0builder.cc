...
dberr_t Builder::copy_columns(Copy_ctx &ctx, size_t &mv_rows_added,
  ...
  for (size_t i = 0; i < ctx.m_n_fields; ++i, ++field, ++ifield) {
    ...
    /* Process the hidden vec_idx_id column, mirroring FTS's
    ...
    source table already has vec_idx_id, fall through to the normal
    ...
    Note: a rebuild with a SURVIVING vector index is refused at
    check_if_supported_inplace_alter (FTS parity), so the copy case
    here only arises for tables that retain vec_idx_id without a
    vector index (Option A retention after DROP INDEX). */
    const bool is_new_vec_idx_id_col =
        DICT_TF2_FLAG_IS_SET(m_index->table, DICT_TF2_VEC_HAS_IDX_ID) &&
        !DICT_TF2_FLAG_IS_SET(m_ctx.m_old_table, DICT_TF2_VEC_HAS_IDX_ID) &&
        col_no == m_index->table->vec_idx_id_col && !col->is_virtual();

    if (is_new_vec_idx_id_col) {
      const uint64_t id = vec_assign_next_idx_id(m_index->table);
