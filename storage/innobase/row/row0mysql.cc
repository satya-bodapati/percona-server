                              ...
                              multi_value & blob data */
{
  ...
  /* Same pattern for the hidden vec_idx_id column (vector indexes).
  ...
  vec_stamp_idx_id(prebuilt->table, row, prebuilt->heap);
}
...
dberr_t row_rename_table_for_mysql(const char *old_name, const char *new_name,
  ...
  if (err == DB_SUCCESS &&
      DICT_TF2_FLAG_IS_SET(table, DICT_TF2_VEC_HAS_IDX_ID) &&
      !dict_tables_have_same_db(old_name, new_name)) {
