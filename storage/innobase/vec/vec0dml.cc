...
dberr_t vec_aux_insert(trx_t *trx, dict_table_t *aux,
  ...
  if (row.ver == 0) {
    ...
    /* A version row normally carries neighbors only. The PK-only
    UPDATE (H1-3) also sets row_ref: the loader takes the LATEST
    VISIBLE non-NULL row_ref per label, so an override row repoints the
    node without touching the birth row. */
...
/** DELETE one aux row by a filled PK search tuple — the shared body of
the delete shapes (meta, posting, dead).

Self-positioned upd_node with node->is_delete: a plain row DELETE, so
retirement/GC is MVCC + purge (design §2), never a status flag. In a
regular SQL DELETE the preceding row_search_mvcc read positions the
cursor and takes the locks; we know the PK and skip the search, so we
position the cursor and take the locks that row_upd_clust_step asserts
(IX on the table, explicit X on the record) ourselves, retrying lock
waits through row_mysql_handle_errors. Implementation reference: the
FK-cascade code, row0ins.cc:1153; its run loop touches thr->prebuilt,
which we do not have — hence the private loop below.

`fill_key` writes the n_key PK fields; the data it points at must
outlive this call. */
...
(H1-4, reload-time collapse). Each entry is a (label, ver) PK.
...
current base-row reference, and the loader resolves row_ref as the
latest visible NON-NULL value. Collapsing such a row because a higher
...
therefore excludes them and this function only executes the list.

Delete-marking is MVCC-safe with no extra bookkeeping: a reader whose
view predates the winning version's commit also predates these marks,
so it still sees the row it needs; newer readers take the winner and
skip the marked rows. Purge reclaims the space afterwards.
...
dberr_t vec_aux_collapse_versions(
  ...
  for (const auto &lv : losers) {
    ...
    const dberr_t err = vec_spann_delete_by_pk(trx, aux, 2, [&](dtuple_t *ref) {
...
dberr_t vec_aux_load_rows(
  ...
  *saw_invisible = false;
  ...
  while (pcur.move_to_next_user_rec(&mtr) == DB_SUCCESS) {
    ...
    /* Raw rows still live in the log — births plus every snapshot not
    yet collapsed. Exposed so tests can observe collapse reclaiming
    them (the resolved count below says nothing about log size). */
    if (n_raw_rows != nullptr) {
      ++(*n_raw_rows);
    }
    ...
    /* row_ref: the LATEST VISIBLE non-NULL value wins (H1-3). The birth
    row supplies it; a PK-only UPDATE appends an override row carrying a
    new one. Ascending ver order makes overwrite the whole rule. */
    ...
    ascending ver order means plain overwrite keeps the highest one. */
    {
      ...
      (H1-4) or is the birth row (identity). */
    ...
    if (ver == 0) {
      /* Birth row: the identity payload. Deadness is NOT here — it
      lives in the _dead table (H1-2); a birth row with no row_ref
      predates that and is treated as carrying none. */
      acc.have_birth = true;
