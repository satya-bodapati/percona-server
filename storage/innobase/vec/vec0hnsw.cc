...
dberr_t vec_knn_search(dict_index_t *index, const float *q, size_t k,
  ...
  {
    /* TODO REMOVE with vec_t::graph_latch. k_nn_search() mutates: it
    faults unloaded stubs in through load_node_cb as it traverses, so a
    search is a writer as far as the graph is concerned. */
    std::lock_guard<std::mutex> graph_guard(vec->graph_latch);
