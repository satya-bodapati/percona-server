...
dberr_t vec_build_write_aux(Vec_build *b, trx_t *trx, dict_table_t *table,
  ...
  b->graph->for_each_node_sorted([&](uint64_t id, uint64_t base_pk,
    ...
    /* Ending the walk on the first failure: every later node would be
    written into a loader that has already failed, and the graph can hold
    millions of them. The error is kept and handed to
    vec_aux_bulk_finish below, which is what rolls the load back. */
